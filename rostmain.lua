-- Voidrane.cc - LINORIA LIBRARY VERSION (ALL FEATURES + WORKING SILENT AIM)
...
-- ========== SILENT AIM (WORKING VERSION FROM ORIGINAL SCRIPT) ==========
local SilentAimSettings = { Enabled = false, TargetPart = "Head", SilentAimMethod = "Raycast", FOVRadius = 130, FOVVisible = false, HitChance = 100 }
...
local function applyHSChar(ch)
    if not ch then return end
    for _, c in pairs(ch:GetChildren()) do if c:IsA("Tool") then chkTool(c) end end
end
...
local physConn = RunService.Heartbeat:Connect(function()
    ...
    if SpeedSettings.Enabled then
        ...
        if md.Magnitude > 0 then hrp.AssemblyLinearVelocity = Vector3.new(md.X * SpeedSettings.Value, hrp.AssemblyLinearVelocity.Y, md.Z * SpeedSettings.Value) end
    end
    if WaterSpeed.Enabled and hum:GetState() == Enum.HumanoidStateType.Swimming then
        local bv = hrp:FindFirstChild("WSV")
        if not bv then bv = Instance.new("BodyVelocity", hrp); bv.Name = "WSV"; bv.MaxForce = Vector3.new(1e5, 1e5, 1e5) end
...
local function createTrace(sp, ep)
    ...
    if b then b.Color = ColorSequence.new(TracerSettings.Color); b.Transparency = NumberSequence.new(TracerSettings.Transparency)
        b.Texture = TracerSettings.TextureID; b.Width0 = TracerSettings.Width; b.Width1 = TracerSettings.Width; b.Enabled = true end
    task.delay(TracerSettings.LifeTime, function() if p then if b then b.Enabled = false end; p.Parent = nil; trSize = trSize + 1; trPool[trSize] = p end end)
end
...
-- ========== ARM/WEAPON CHAMS ==========
local armChamsColor = Color3.fromRGB(64, 224, 208)
local weaponChamsColor = Color3.fromRGB(30, 144, 255)
...
-- ========== HEMP ESP ==========
local HempESPSettings = { Enabled = false, Radius = 300, Color = Color3.fromRGB(0, 255, 0) }
local hempHL = {}
local function updateHempESP()
    ...
    for _, obj in ipairs(hf:GetDescendants()) do
        if obj:IsA("Model") or obj:IsA("BasePart") then
            local op = obj:IsA("Model") and obj:GetPivot().Position or obj.Position
            if (op - pp).Magnitude <= HempESPSettings.Radius then
                local hl = Instance.new("Highlight"); hl.FillColor = HempESPSettings.Color; hl.OutlineColor = HempESPSettings.Color
                hl.FillTransparency = 0.5; hl.OutlineTransparency = 0.3; hl.Enabled = true
...
-- ========== HITBOX EXPANDER (WALLHACK) ==========
local HitboxSettings = { Enabled = false, Size = Vector3.new(5, 5, 5), Show = false, Color = Color3.fromRGB(0, 100, 255) }
local origSizes = {}
local function applyHitbox()
    for _, pl in ipairs(Players:GetPlayers()) do if pl ~= LocalPlayer and pl.Character then for _, p in ipairs(pl.Character:GetChildren()) do
        if p:IsA("BasePart") and p.Name ~= "HumanoidRootPart" then
            if not origSizes[p] then origSizes[p] = p.Size end
            p.Size = origSizes[p] + HitboxSettings.Size; p.CanCollide = false
            if HitboxSettings.Show then
                local hl = p:FindFirstChild("VH"); if not hl then hl = Instance.new("Highlight", p); hl.Name = "VH"; hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop end
                hl.FillColor = HitboxSettings.Color; hl.OutlineColor = HitboxSettings.Color; hl.FillTransparency = 0.5; hl.OutlineTransparency = 0.5; hl.Enabled = true
            end
        end
    end end end
end
local function restoreHitbox()
    for p, s in pairs(origSizes) do if p and p.Parent then p.Size = s; p.CanCollide = true end; local hl = p and p:FindFirstChild("VH"); if hl then hl:Destroy() end end; origSizes = {}
end
...
local function toggleSpikes(v)
    if v then
        for _, o in ipairs(workspace:GetDescendants()) do if o.Name == "Spikes" then pcall(function() o:Destroy() end) end end
...
local function createModList()
    ...
    local lc = Instance.new("Frame", mf); lc.Size = UDim2.new(1, 0, 1, -25); lc.Position = UDim2.new(0, 0, 0, 25); lc.BackgroundTransparency = 1
    Instance.new("UIListLayout", lc).SortOrder = Enum.SortOrder.LayoutOrder
    for _, pl in ipairs(Players:GetPlayers()) do if pl ~= LocalPlayer then
        local s, r = pcall(function() return pl:GetRoleInGroup(15631191) end)
...
if LocalPlayer.Character then setupJC(LocalPlayer.Character) end
LocalPlayer.CharacterAdded:Connect(setupJC)
...
do
    ...
    function Esp:ConvertScreenPoint(wp)
        local vs = Camera.ViewportSize; local lp = Camera.CFrame:pointToObjectSpace(wp); local ar = vs.X / vs.Y
        local hy = -lp.Z * math.tan(math.rad(Camera.FieldOfView / 2)); local hx = ar * hy
        local fc = Vector3.new(-hx, hy, lp.Z); local rp = lp - fc; local sx = rp.X / (hx * 2); local sy = -rp.Y / (hy * 2)
        local os = -lp.Z > 0 and sx >= 0 and sx <= 1 and sy >= 0 and sy <= 1
        return Vector3.new(sx * vs.X, sy * vs.Y, -lp.Z), os
    end
    ...
    function Esp:BoxSolve(torso)
        ...
        local vt = torso.Position + (torso.CFrame.UpVector * 1.8) + Camera.CFrame.UpVector
        local vb = torso.Position - (torso.CFrame.UpVector * 2.5) - Camera.CFrame.UpVector
        local d = (torso.Position - Camera.CFrame.p).Magnitude; local nd = math.floor(d * 0.333)
        local t, tir = Esp:ConvertScreenPoint(vt); local b, bir = Esp:ConvertScreenPoint(vb)
        if not tir or not bir then return nil, nil, false, nd end
        local w = math.max(math.floor(math.abs(t.X - b.X)), 8); local h = math.max(math.floor(math.max(math.abs(b.Y - t.Y), w / 2)), 12)
        return Vector2.new(math.floor(math.max(h / 1.5, w)), h), Vector2.new(math.floor(t.X * 0.5 + b.X * 0.5 - math.floor(math.max(h / 1.5, w)) * 0.5), math.floor(math.min(t.Y, b.Y))), true, nd
    end
    function Esp.CreateObject(player)
        local Data = { Items = {}, Info = {Character = nil, Humanoid = nil, Health = 0} }; local Items = Data.Items
    ...
    function Esp.Update()
        if not Esp or not MiscOptions.Enabled then return end
        for _, Data in Esp.Players do
            if not Data.Info or not Data.Info.Character or not Data.Info.Humanoid then continue end
            local Items = Data.Items; if not Items then continue end
            local bs, bp, os, d = Esp:BoxSolve(Data.Info.Humanoid.RootPart); local h = Items["Holder"]; if not h then continue end
            if d == nil or not os or d > MiscOptions["Render Distance"] then if h.Visible then h.Visible = false end; continue end
            h.Visible = true; h.Position = UDim2.fromOffset(bp.X, bp.Y); h.Size = UDim2.new(0, bs.X, 0, bs.Y)
            Items.DistanceText.Text = tostring(math.round(d)) .. "m"
            Items.Box.Visible = MiscOptions.Boxes; Items.Healthbar.Visible = MiscOptions.Healthbar; Items.NameText.Visible = MiscOptions.Name_Text; Items.DistanceText.Visible = MiscOptions.Distance_Text; Items.WeaponText.Visible = MiscOptions.Weapon_Text
        end
    end
    function Esp.RefreshElements(k, v)
        for _, Data in Esp.Players do local Items = Data and Data.Items; if not Items or not Items.Holder then continue end
            if k == "Enabled" then Items.Holder.Visible = v elseif k == "Boxes" then Items.Box.Visible = v elseif k == "Healthbar" then Items.Healthbar.Visible = v elseif k == "Name_Text" then Items.NameText.Visible = v elseif k == "Distance_Text" then Items.DistanceText.Visible = v elseif k == "Weapon_Text" then Items.WeaponText.Visible = v elseif k == "ChamsEnabled" and Data.Highlight then Data.Highlight.Enabled = v elseif k == "Chams Fill" and Data.Highlight then Data.Highlight.FillColor = v.Color; Data.Highlight.FillTransparency = 1 - v.Transparency elseif k == "Chams Outline" and Data.Highlight then Data.Highlight.OutlineColor = v.Color; Data.Highlight.OutlineTransparency = 1 - v.Transparency end
        end
    end
...
task.spawn(function()
    ...
    while true do
        ...
        FOVCircle.Position = mp; FOVCircle.Radius = SilentAimSettings.FOVRadius
        FOVCircle.Color = FOVCircleSettings.Color; FOVCircle.Filled = FOVCircleSettings.Filled
        if FOVCircleSettings.Filled then FOVCircle.Transparency = FOVCircleSettings.FilledTransparency end
        FOVCircle.Thickness = FOVCircleSettings.OutlineThickness; FOVCircle.NumSides = FOVCircleSettings.Sides
...
local FOVCircleGroup = Tabs.Combat:AddLeftGroupbox('FOV Circle')
...
print("Voidrane.cc - ALL FEATURES + WORKING SILENT AIM! Press END to toggle GUI")
